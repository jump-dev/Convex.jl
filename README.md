...
The `master` branch of this package (not yet released)
contains an experimental JuMP solver.
This solver reformulates a nonlinear JuMP model into a conic
program using DCP.
Note that it currently only supports scalar nonlinear programs.
Note that `add_bridges = false` should be used as otherwise two consecutive layers of bridges are used which creates an issue.
```julia
