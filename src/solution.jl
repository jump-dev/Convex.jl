...
function populate_variables!(problem::Problem, var_to_ranges::Dict{UInt64, Tuple{Int, Int}})
  ...
  for (id, (start_index, end_index)) in var_to_ranges
    ...
    if var.sign != ComplexSign()
      ...
      real_value = reshape(x[start_index:start_index + (end_index-start_index+1)/2-1], sz[1], sz[2])
      imag_value = reshape(x[start_index + (end_index-start_index+1)/2:end_index], sz[1], sz[2])
