...
function get_MOI_set(cone, length_inds)
    if cone == :SDP
        set = MOI.PositiveSemidefiniteConeTriangle(
            Int(sqrt(0.25 + 2 * length_inds) - 0.5),
        )
    elseif cone == :Zero
...
function add_terms!(
    ...
    I, J, V = findnz(matrix)
    for k in eachindex(I, J, V)
        term = MOI.VectorAffineTerm{T}(
            CIs[I[k], J[k]][1],
            MOI.ScalarAffineTerm{T}(V[k], var_indices[CIs[I[k], J[k]][2]]),
        )
        push!(terms, term)
    end
end
...
function add_terms!(
    ...
    CIs = CartesianIndices((row_indices, 1:length(var_indices)))
    I, V = findnz(vector)
    for k in eachindex(I, V)
        term = MOI.VectorAffineTerm{T}(
            CIs[I[k]][1],
            MOI.ScalarAffineTerm{T}(V[k], var_indices[CIs[I[k]][2]]),
        )
        push!(terms, term)
    end
end
...
function add_terms!(
    ...
    CIs = CartesianIndices((row_indices, 1:length(var_indices)))
    for ci in CartesianIndices(size(matrix))
        term = MOI.VectorAffineTerm{T}(
            CIs[ci][1],
            MOI.ScalarAffineTerm{T}(matrix[ci], var_indices[CIs[ci][2]]),
        )
        push!(terms, term)
    end
end
...
function add_terms!(
    ...
    for k in 1:length(vector)
        term = MOI.VectorAffineTerm{T}(
            CIs[k][1],
            MOI.ScalarAffineTerm{T}(vector[k], var_indices[CIs[k][2]]),
        )
        push!(terms, term)
    end
end
...
function make_MOI_constr(
    conic_constr::ConicConstr,
    var_to_indices,
    id_to_variables,
    T,
)
            ...
            if id == objectid(:constant)
                ...
                if sign(id_to_variables[id]) == ComplexSign()
                    ...
                    add_terms!(
                        terms,
                        val[1],
                        constr_index+1:constr_index+sz,
                        var_indices[1:l],
                    )
...
function load_MOI_model!(model, problem::Problem{T}) where {T}
    ...
    var_to_indices =
        get_variable_indices!(model, conic_constraints, id_to_variables)
    ...
    MOI.set(
        model,
        MOI.ObjectiveFunction{SV_OR_VI}(),
        SV_OR_IDENTITY(objective_index),
    )
    MOI.set(
        model,
        MOI.ObjectiveSense(),
        problem.head == :maximize ? MOI.MAX_SENSE : MOI.MIN_SENSE,
    )
    ...
    for conic_constr in conic_constraints
        set, constr_fn =
            make_MOI_constr(conic_constr, var_to_indices, id_to_variables, T)
        push!(MOI_sets, set)
...
function solve!(
    ...
    conic_constraints,
    var_to_indices,
    constraint_indices = load_MOI_model!(model, problem)
...
function moi_populate_solution!(
    model::MOI.ModelLike,
    problem,
    id_to_variables,
    conic_constr_to_constr,
    ...
    if dual_status != MOI.NO_SOLUTION
        for (idx, conic_constr) in enumerate(conic_constraints)
            ...
            dual_value_vectorized =
                MOI.get(model, MOI.ConstraintDual(), MOI_constr_indices)
            iscomplex =
                sign(constr.lhs) == ComplexSign() ||
                sign(constr.rhs) == ComplexSign()
            constr.dual =
                unpackvec(dual_value_vectorized, constr.size, iscomplex)
        end
    else
        for (idx, conic_constr) in enumerate(conic_constraints)
            haskey(conic_constr_to_constr, conic_constr) || continue
