...
Variable(size::Integer; kwargs...) = Variable((size, 1),:any; kwargs...)
Variable(size::Integer,sign::Symbol; kwargs...) = Variable((size, 1),sign; kwargs...)
...
type Constant <: AbstractCvxExpr
  ...
  function Constant(x::Value,sign)
    if sign in signs
      ...
      # g[1:5,1:1]=a
      # causes an error
      # Once julia fixes it, we can get back to x
