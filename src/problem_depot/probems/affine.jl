...
@add_problem affine function Partial_transpose(handle_problem!, valtest::Val{test} = Val(false), atol=1e-4, rtol=0.0, typ::Type{T} = Float64) where {T, test}
    ...
    handle_problem!(satisfy(partialtranspose(Rt1, 1, dims) == S ))
    handle_problem!(satisfy(partialtranspose(Rt2, 2, dims) == S ))
    handle_problem!(satisfy(partialtranspose(Rt3, 3, dims) == S ))
