...
+----------------------+-------------------------+------------+---------------+-------------------------+
|:code:`inv_pos(x,y)`  | :math:`1/max(x,0)`      | convex     |decreasing     | :math:`x>0`             |
+----------------------+-------------------------+------------+---------------+-------------------------+
...
.. atom | description | vexity | slope | implicit constraint
.. -----|--------|--------|------- |-------
.. `norm(x, p)` | :math:$(\sum x_i^p)^{1/p}$ | convex | increasing on :math:$x \ge 0$, decreasing on :math:$x \le 0$ | `p = 1, 2, Inf`
.. `vecnorm(x, p)` | :math:$(\sum x_{ij}^p)^{1/p}$ | convex | increasing on :math:$x \ge 0$, decreasing on :math:$x \le 0$ | `p = 1, 2, Inf`
.. `quad_form(x,P)` | :math:$x^T P x$ | convex in :math:$X$, affine in :math:$P$ | increasing on :math:$x \ge 0$, decreasing on :math:$x \le 0$, increasing in :math:$P$ | either :math:$x$ or :math:$P$ must be constant
.. `quad_over_lin(x, y)` | :math:$x^T x/y$ | convex | increasing on :math:$x \ge 0$, decreasing on :math:$x \le 0$, decreasing in :math:$y$ | :math:$y > 0$
.. `sum_squares(x)` | :math:$\sum x_i^2$ | convex | increasing on :math:$x \ge 0$, decreasing on :math:$x \le 0$ | none
.. `nuclear_norm(x)` | sum of singular values of :math:$x$ | convex | not monotonic | none
.. `operator_norm(x)` | maximum singular value of :math:$x$ | convex | not monotonic | none
.. `lambda_max(x)` | maximum eigenvalue of :math:$x$ | convex | increasing | x is positive semidefinite
.. `lambda_min(x)` | minimum eigenvalue of :math:$x$ | concave | increasing | x is positive semidefinite
.. `logsumexp(x)` | :math:$\log(\sum_i \exp(x_i))$ | convex | increasing | none


Promotions
***********

When an atom or constraint is applied to a scalar and a higher dimensional variable, the scalars are promoted. For example, we can do :code:`max(x, 0)` gives an expression with the shape of :code:`x` whose elements are the maximum of the corresponding element of :cdoe:`x` and :code:`0`.
